...
export const onWikiRevision = async (event: ModAction, context: TriggerContext) => {
    ...
    if (processing) {
        console.warn('onWikiRevision', 'queue is already being processed');
        return;
    }
    ...
    console.debug('onWikiRevision', 'scheduled processing of the queue for one second from now');
};
...
export const onProcessMessageQueue = async (event: ScheduledJobEvent<Nothing>, context: TriggerContext) => {
    ...
    if (members.length === 0) {
        console.debug('onProcessMessageQueue', 'the queue is empty and kitty is pleased');
    ...
    console.debug('onProcessMessageQueue', 'scheduled additional processing of the queue for one second from now');
};
